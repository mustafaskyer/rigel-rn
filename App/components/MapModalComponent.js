...
const SearchItem = ({ name, formatted_address, geometry, icon, _handleConfirm }) => {
  const {
    location: { lat, lng }
  } = geometry;
  return (
    <TouchableWithoutFeedback onPress={() => _handleConfirm({ name, formatted_address, lat, lng, icon })}>
    ...
    <Modal {...props} style={styles.container}>
      <Header style={{ height: 100 }}>
        ...
        <Row style={{ paddingStart: 16 }}>
          ...
          <TouchableWithoutFeedback onPress={() => setIsVisible( state => state = true)}>
            <Text style={styles.input}>{"search"}</Text>
          </TouchableWithoutFeedback>
        </Row>
      </Header>
      <MapBoxGl
      coords={place ? [parseFloat(place.lng), parseFloat(place.lat)] : [position.coords.longitude,position.coords.latitude]}
      ...
      <Modal isVisible={isVisible} style={styles.container}>
        ...
        <View style={{ flex: 1, backgroundColor: "#F0F0F2" }}>
          ...
          <FlatList
            style={{ flex: 1 }}
            data={data}
            renderItem={({ item }) => <SearchItem _handleConfirm={res => {
